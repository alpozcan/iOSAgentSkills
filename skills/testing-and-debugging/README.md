...
[[05-swift-testing-and-tdd-patterns]] is the foundation — Swift Testing framework (`@Suite`, `@Test`, `#expect`) over XCTest for all unit tests. It defines five test categories (service logic, catalog, AI engine, sanitizer/filter, actor-based async) and the test isolation pattern using `UserDefaults(suiteName: #function)`. The stub injection strategy here works hand-in-hand with [[09-debug-modes-and-mock-service-strategy]].
