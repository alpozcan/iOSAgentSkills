...
public actor AnalyticsService: AnalyticsServiceProtocol {
    ...
    private func _track(_ event: AnalyticsEvent) async {
        ...
        for backend in backends { await backend.track(event) }
    }
...
```swift
public struct AnalyticsServiceSpec: DependencySpec {
    public typealias Service = AnalyticsServiceProtocol
    
    public static func make(resolver: DependencyResolving) -> AnalyticsServiceProtocol {
        ...
        return AnalyticsService(backends: backends)
    }
}
```
