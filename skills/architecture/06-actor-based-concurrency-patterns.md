...
public final class ServiceProvider: DependencyResolving, @unchecked Sendable {
    ...
    public func register<Spec>(_ specType: Spec.Type, factory: @escaping (ServiceProvider) -> Spec.Service) {
        lock.lock()
...
- **`@unchecked Sendable` with `NSLock`** is the right choice for DI containers that need synchronous access without `await`
